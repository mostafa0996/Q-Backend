  ...
  schema.statics.paginate = async function (filter, options, req) {
    ...
    if (!req.user || ((req.user.role !== admin) && (req.user.role !== master))) {
      ...
      agrigateFilters.push({
        '$project': {
          content: '$content_' + req.headers['accept-language'] ? '$content_' + req.headers['accept-language'] : '$content_' + 'en',
          title: '$title_' + req.headers['accept-language'] ? '$title_' + req.headers['accept-language'] : '$title_' + 'en',
          cover: 1, type: 1, image: 1, value: 1
        }
      })
